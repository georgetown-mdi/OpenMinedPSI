...
const outputs = [
  ...
  {
    // Native (Node-only) N-API build. Same implementation/ layer as the Node
    // WASM bundle, but the raw module comes from the prebuilt .node addon
    // resolved at runtime by node-gyp-build (kept external so it is required
    // from node_modules, not inlined). The prebuilt binaries are shipped
    // alongside this bundle under prebuilds/<platform>-<arch>/.
    input: `private_set_intersection/javascript/src/native_node.ts`,
    onwarn(warning, warn) {
      // suppress eval warnings from google-protobuf
      if (warning.code === 'EVAL') return
      warn(warning)
    ...
    output: {
      file: `private_set_intersection/javascript/dist/psi_native_node.js`,
      sourcemap: true,
      format: 'cjs',
      name: 'PSI',
      exports: 'auto',
